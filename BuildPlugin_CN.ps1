...
function Test-UE {
    Write-Host "[1/4] 检查 UE 5.4..." -ForegroundColor Yellow
    if (Test-Path $UATPath) {
        Write-Host "  [成功] UE 5.4 已找到" -ForegroundColor Green
        return $true
    } else {
        Write-Host "  [错误] 未找到 UE 5.4" -ForegroundColor Red
        Write-Host "  请确认 UE 5.4 安装在: $UEPath" -ForegroundColor Yellow
        return $false
    }
}
